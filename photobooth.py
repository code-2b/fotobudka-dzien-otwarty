...
STATS_DB_PATH = os.path.join(os.path.dirname(__file__), "photobooth_stats.sqlite3")


def _ensure_stats_db() -> None:
    with sqlite3.connect(STATS_DB_PATH, timeout=5) as conn:
        conn.execute(
            """
        ...
        conn.execute(
            "INSERT OR IGNORE INTO counters(name, value) VALUES('all_photos', 0)"
        )


def _get_global_photo_count() -> int | None:
    try:
        _ensure_stats_db()
        with sqlite3.connect(STATS_DB_PATH, timeout=5) as conn:
            row = conn.execute(
                "SELECT value FROM counters WHERE name = 'all_photos'"
            ).fetchone()
...
def _increment_global_photo_count() -> int | None:
    try:
        _ensure_stats_db()
        with sqlite3.connect(STATS_DB_PATH, timeout=5) as conn:
            conn.execute(
                "UPDATE counters SET value = value + 1 WHERE name = 'all_photos'"
            )
            row = conn.execute(
                "SELECT value FROM counters WHERE name = 'all_photos'"
            ).fetchone()
...
def _update_capture_counter(uploaded_file) -> tuple[int, int | None]:
    """Licznik sesyjny + globalny licznik wszystkich zdjęć."""
    if "photo_count" not in st.session_state:
        st.session_state.photo_count = 0
    if "seen_photo_hashes" not in st.session_state:
        st.session_state.seen_photo_hashes = set()

    global_count = _get_global_photo_count()

    if uploaded_file is None:
        return st.session_state.photo_count, global_count
    ...
    if digest not in st.session_state.seen_photo_hashes:
        st.session_state.seen_photo_hashes.add(digest)
        st.session_state.photo_count += 1
        incremented = _increment_global_photo_count()
        if incremented is not None:
            global_count = incremented
    return st.session_state.photo_count, global_count
...
if total_photo_count is None:
    ...
    st.caption(
        f"Licznik zdjęć: sesja {photo_count} | wszystkie zdjęcia w aplikacji: {total_photo_count}"
    )
...
def _load_font(size: int, bold: bool = False) -> ImageFont.ImageFont:
    ...
    font_candidates = bold_candidates + regular_candidates if bold else regular_candidates
...
def _paste_logo_top_left(photo_rgba: Image.Image, thickness: int):
    ...
    # Zamiana logo na białe przy zachowaniu kanału alfa.
    alpha = logo.split()[-1]
    ...
    base_target_w = max(120, min(int(w * 0.22), 280))
    target_w = int(base_target_w * 1.3)
    target_w = min(target_w, max(80, w - 2 * margin))
...
def _draw_caption_and_qr(photo: Image.Image, caption: str, qr_text: str | None):
    ...
    font_size = max(11, h // 40)
    min_font_size = 8
    chosen_lines: list[str] = []
    chosen_font: ImageFont.ImageFont = _load_font(font_size, bold=True)
    ...
    if not chosen_lines:
        chosen_font = _load_font(min_font_size, bold=True)
